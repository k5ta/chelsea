...
export default {
  ...
  methods: {
    ...
    orderingFormEnterAnimation(el, done) {
      const timeline = new TimelineLite({ onComplete: done })
      const s = this.s

      timeline
        .fromTo(el,0.5,{ autoAlpha: 0},{  autoAlpha: 1},0)
        .fromTo(`.${s.orderingForm}`,0.5,{ autoAlpha: 0, scale: 0 },{  autoAlpha: 1, scale: 1 },0)
    },
    orderingFormLeaveAnimation(el, done) {
      const timeline = new TimelineLite({ onComplete: done })
      const s = this.s

      timeline
        .fromTo(`.${s.orderingForm}`,0.5,{ autoAlpha: 1, scale: 1 },{  autoAlpha: 0, scale: 0 },0)
